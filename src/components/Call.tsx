...
import { GroundTruthTrade, GroundTruthVehicle, PersonaAvatar, Sheet, SentimentBar, ease } from "./ui";
...
export function Call({
  ...
  const [transcript, setTranscript] = useState<TranscriptRow[]>([
    { who: "customer", text: persona.nodes[persona.startNode].customerLine, nodeId: persona.startNode },
  ]);
  ...
  function pick(choice: Choice) {
    ...
    setPicks(newPicks);
    setSheetOpen(false);
    setTranscript((t) => [...t, { who: "rep", text: choice.text, nodeId }]);
    setRepFlash(choice.text);
    ...
    setTimeout(() => {
      ...
      setTranscript((t) => [...t, { who: "customer", text: nextNode.customerLine, nodeId: choice.next as string }]);
    }, 1300);
  }

  const gt = node?.groundTruth;
  const ov = overall(sentiment);
  const heroText = repFlash ?? node?.customerLine ?? "";
  ...
  return (
    <div className="coach-dark flex h-full flex-col">
      ...
      <div className="shrink-0 px-4 py-3">
        <SentimentBar value={ov} dark threshold={LOST_THRESHOLD} flashKey={flashKey} flashTone={flashTone} />
      </div>
